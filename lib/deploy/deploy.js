...
const deploy = ( payload={}, app_path=null, downstream_path=null, cb=()=>{}) => {

	const {
		repository: { name: repo_name = null } = { name: null }
	} = payload;

	console.log(`Pull request ${payload.action} and ${payload.pull_request.state}`)
	...
	const pull = `${git} pull origin master -f`;
